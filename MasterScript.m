% This MASTER SCRIPT has been used to produce the analysis presented in Barrington et al. (2022) Wind speed as a dominant source of periodicities in reported emission rates of volcanic SO2. 
 
% This script runs a series of functions to analyse time series data from the Network of Observation of Volcanic and Atmospheric Change (NOVAC) database for the purpose of identifying periodic trends in daily observed SO2 emission rates. 
 
% Created by C. BARRINGTON, March 2022.
% Modifications have not been logged. 
 
% DISCLAIMER: This MASTER SCRIPT has been used to produce the analysis presented in "Barrington et al. (2022) Wind speed as a dominant source of periodicities in reported emission rates of volcanic SO2" and has been provided to in the interest of research integrity. 
% This version of the code is not thoroughly commented nor is it free of redundancies. Figures produced using this code have been modified and formatted independently but the data unchanged. Users wishing to apply similar analysis to their own datasets, are directed 
% to use the user-friendly snippets of code that are provided. Some lines of code (including those within functions) are required to be commented/uncommented depending on whether the figure contains results from a single volcano or multiple subplots. It is therefore 
% advisable to run each section of the code separately. Not all hardcoding has been removed. 
...
% Runs 'plotPSDplumespeed' function to PLOTS THE PSD ESTIMATES OF SO2 EMISSION RATE AGAINST THE PSD ESTIMATES OF PLUME SPEED
        % Function saves figure in the directory 'outDirFig', <volcano_PSDxy.fig>
             ...
             % HARDCODING 
                    ...
                    outDir = '<insert path to output directory>’; % Directory to output folder for saving results file';
